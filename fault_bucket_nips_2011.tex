...
\title{A Learning Method for Pattern Detection, Cleaning and
...
\citep{wagner2006guidelines}. To accomplish this, we need to design
techniques for detecting patterns and making predictions with
challenging datasets, fraught with missing data, discontinuities and
...
In this paper, we attack the problem of fault detection, correction
...
unanticipated exogenous disturbances.
...
% \begin{figure*}
% \begin{center}
% %\includegraphics[height=.5\textwidth]{waterMonitoring.pdf}
% \end{center}
% \caption{{\small \it The water sustainability challenge. The
% encountered time series are highly nonstationary, with varying
% length-scales, cycles, drift, offsets, spikes, missing observations,
% and many unanticipated anomalies. STATE THAT FOR FISHKILLER, WE'RE
...
inapplicable. Further, many of these are too restrictive
(\emph{e.g.} use linear, Markov or Gaussian assumptions) or fail to
...
we focus on developing \acro{gp}-based
techniques to build probabilistic nonlinear models of the signal.  In
addition to
...
in highly variable or poorly understood environments. As a result of
much experimentation (some of the results with alternative techniques
...
monitoring, for this reason we developed of a catch-all ``fault
bucket,'' which can identify and treat
appropriately data readings suspected of being corrupted in some way.
Notably, we do not require the specification of precise fault models.
...
% In the proposed  ``fault bucket'' approach, each point is considered
% to have been generated from either a nominal or faulty process. In
% this way, our model can simultaneously identify anomalies and robustly
% make predictions in the presence of sensor faults. Since we cannot
...
\begin{enumerate}
 \item \label{app:fb} {\bf Fault bucket:} Faulty observations are assumed to be generated from a Gaussian noise distribution with a very wide variance (as in \cite{Dereszynski}).
...
\begin{figure*}[t]
 ...
 \small
 \begin{tabular}{cc}
   \hspace*{2em} \textsc{xgp} -- ``fishkiller'' &
   \hspace*{2em} \textsc{xgp} -- painting \\
%\vspace{-0.5cm}
   \psff{fk_exhaust} & \psff{painting_exhaust} \\
   \hspace*{2em} \textsc{fb} -- ``fishkiller'' &
   \hspace*{2em} \textsc{fb} -- painting \\
   \psff{fk} & \psff{painting} \\
   %\hspace*{1.2em} 
\psff{fk_fault} &
   %\hspace*{0.5em} 
\psff{painting_fault}
 ...
 \caption{Mean and $\pm3\sigma$ standard-deviation bounds for the
   ...
   (\textsc{fb}) algorithms on the ``fishkiller'' and painting
   datasets.  For the fault-bucket algorithm, the posterior
   faultiness of each observation is also shown underneath the
   predictions.  Note that each column shares the same $x$-axis.}
 \label{justfb}
\end{figure*}
...
flood.
%\subsubsection{pH}
Our next (real) dataset deals with pH measurements from the United States Geological Survey, a common indicator of water quality. In this series, a clear sensor fault can be seen
in which the observation undergo a sudden decrease in value.
...
%\subsection{Results}
Figures \ref{compare} show the performance of the
fault-bucket algorithm on two real datasets, as well as the
performance of the exhaustive .  In all cases, the fault-bucket algorithm did an excellent
