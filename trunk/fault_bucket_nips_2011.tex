...
We propose an algorithm that is designed to deal with faults of many different, unspecified types. We use a sequential scheme, applicable for ordered data such as time series, partitioning the data available at any point into old and new halves.  We then approximately marginalise the faultinesses of old observations, storing and updated our results for future use. This gives rise to an efficient and fast algorithm. In order to effect our scheme, we make four key approximations.
\begin{enumerate}
...
\item \label{app:single_gaussian} {\bf Single Gaussian marginalisation:} Sums of Gaussians weighted by the posterior probabilities of faultiness of old data are approximated as single Gaussians.
...
\item \label{app:affine} {\bf Precision affine:} The precision matrix over both old and new halves is assumed to be affine in the precision matrix over the old half.
\end{enumerate}
...
infeasible to marginalise over all these possible values.
...
typically less pertinent for this task than more-recent ones. We hence approximately marginalise the faultiness of old observations, representing the different Gaussian predictions yielded by each combination of faultinesses as a single Gaussian.
...
\end{equation}
and hence the hyperparameter posterior, $\ps{\vs_a}{\data_{a}}$.
% \begin{equation*}
% \ps{\vs_a}{\data_{a}} 
...
\begin{equation*}
\p{f\st}{\data_{a}} = \sum_{i} \Prob{\vs^i_{a}}{\data_a} \p{f\st}{\data_a, \vs^{i}_{a}}
=\sum_{i} \Prob{\vs^i_{a}}{\data_a} \cm{N}\bigl(f\st; \mean{f\st}{\data_a, \vs^{i}_{a}}, \cov{f\st}{\data_a, \vs^{i}_{a}}\bigr),
\end{equation*}
...
\begin{align*}
 &\p{f\st}{\data_{a}} \simeq \cm{N}\bigl(f\st; \amean{f\st}{\data_a}, \acov{f\st}{\data_a}\bigr),%\label{eq:pya}
\end{align*}
where we have
\begin{align}
\amean{f\st}{\data_{a}} \defequal {}& K_{\star,a} \tilde{V}_a^{-1} \vy_a,\label{eq:ameana}\\
\acov{f\st}{\data_{a}}
\defequal {}& K_{\star,\star} - K_{\star,a}(\tilde{V}_a^{-1}-\tilde{W}_a^{-1})K_{a,\star} 
 - \amean{f\st}{\data_{a,b}}^2 ,\label{eq:acova}
\end{align}
where\footnote{
...
\begin{align}
 ...
 \tilde{W}_a^{-1} & \defequal \sum_i \Prob{\vs^i_{a}}{\data_a} (V_a^i)^{-1}\vy_a \vy_a^\tr (V_a^i)^{-1}.\label{eq:Wa}
\end{align}
% Having computed
% $\tilde{V}_a$, we can then calculate
% \begin{align} %labelled only for alg statement
%  \tilde{R}_a & \defequal \chol \tilde{V}_a \\%\label{eq:Ra}, \\ 
%  \tilde{T}_a & \defequal \chol (\tilde{R}_a)^{-1} \vy_a %\label{eq:Ta},
% \end{align}
...
\begin{align*}
\p{f\st}{\data_{a,b}} & = \sum_{i} \sum_{j} \p{f\st}{\data_{a,b}, \vs^{i,j}_{a,b}} \Prob{\vs^{i,j}_{a,b}}{\data_{a,b}}.
\end{align*}
The full sums here can easily become too large to actually
evaluate. To progress, we make Approximation \ref{app:independence}. To be precise, we approximate as
...
giving
\begin{align}
p(f\st \given \data_{a,b})
\simeq 
% {}& \sum_{j} \Prob{\vs^j_{b}}{\data_{a,b}}\sum_{i} \Prob{\vs^i_{a}}{\data_a} \p{f\st}{\data_{a,b}, \vs^{i,j}_{a,b}}\nonumber\\
% = {}&
\sum_{j} \Prob{\vs^j_{b}}{\data_{a,b}}\sum_{i} \Prob{\vs^i_{a}}{\data_a} {}
 \cm{N}\bigl(f\st; \mean{f\st}{\data_{a,b}, \vs^{i,j}_{a,b}}, \cov{f\st}{\data_{a,b}, \vs^{i,j}_{a,b}}\bigr).\label{eq:sum_o_Gaussians}
\end{align}
...
\begin{align*}
\Prob{\vs_b}{\data_{a,b}} & = \frac{\sum_i \p{\vy_b}{\data_a,\vs^i_{a,b}}p(\vy_a,\vs^i_{a,b})}{p(\vy_{a,b})}\nonumber\\
& \simeq \frac{\sum_i \p{\vy_b}{\data_a,\vs^i_{a,b}}\fPr(\vs_a^i\mid{\data_a})\fPr(\vs_{b})}{\ps{\vy_{b}}{\data_a}}\nonumber\\
& \simeq \frac{\cm{N}\bigl(\vy_b; \amean{\vy_b}{\data_a}, \acov{\vy_b}{\data_a, \Sigma_b}\bigr) \fPr(\vs_b)}{\p{\vy_{b}}{\data_a}},%\label{eq:psb}
\end{align*}
where we have
\begin{align}
p(\vy_{b} &\given \data_a)\nonumber\\
& = \sum_i \sum_j \p{\vy_b}{\data_a,\vs^i_{a,b}}\Prob{\vs^{i,j}_{a,b}}{\data_a}\nonumber\\
& \simeq \sum_i \sum_j \p{\vy_b}{\data_a,\vs^i_{a,b}}\Prob{\vs_a^i}{\data_a}\fPr(\vs_{b}^j)\nonumber\\
& \simeq \sum_j \cm{N}\bigl(\vy_b; \amean{\vy_b}{\data_a}, \acov{\vy_b}{\data_a, \vs_b^j}\bigr) \fPr(\vs_b^j).\label{eq:likelihood_b}
\end{align}
...
\begin{align}
\p{f\st}{\data_{a,b}} & \simeq \N{f\st}{\amean{f\st}{\data_{a,b}}}{\acov{f\st}{\data_{a,b}}}\,,\label{eq:pyab}
\end{align}
where we have
\begin{align*}
\amean{f\st}{\data_{a,b}} \defequal {}&  K_{\star,a,b} \tilde{V}_{a,b}^{-1} \vy_{a,b}%\label{eq:ameanab}
\\
\acov{f\st}{\data_{a}}
\defequal {} & K_{\star,\star} - K_{\star,a}(\tilde{V}_{a,b}^{-1}-\tilde{W}_{a,b}^{-1})K_{a,\star} {} \nonumber
\\
& - \amean{f\st}{\data_{a,b}}^2 \,,%\label{eq:acovab}
\end{align*}
\vspace{-0.8cm}
\begin{align*}
...
%\times {} 
\\
& \times \vy_{a,b}^{\phantom{\tr}} \vy_{a,b}^\tr (V_{a,b}^{i,j})^{-1}.
\end{align*}
...
\begin{align}
 ...
 \tilde{T}_{a,b} & \defequal \chol (\tilde{R}_{a,b})^{-1} \vy_a \label{eq:Tab};
\end{align}
...
\begin{align}
\tilde{W}^{-1}_{a,b} \defequal
& \sum_{j} \Prob{\vs^j_{b}}{\data_{a,b}} (\hat{V}_{a,b}^{j})^{-1}\vy_{a,b}^{\phantom{\tr}} \vy_{a,b}^\tr (\hat{V}_{a,b}^{i,j})^{-1}\,.\label{eq:Wab}
\end{align}
...
%  \STATE $\amean{f\st}{\data_a} \leftarrow$ \eqref{eq:ameana}, $\acov{f\st}{\data_a} \leftarrow$ \eqref{eq:acova}\\
% \STATE {\bfseries return:} $\p{f\st}{\data_{a}} \leftarrow$ \eqref{eq:pya}\\
% \FOR {$t = 2,\ldots$}
% \STATE $x_b \leftarrow x_t, z_b \leftarrow z_t, x\st \leftarrow x_{t+\delta}$\\
% \STATE $\amean{\vy_b}{\data_a} \leftarrow$ \eqref{eq;ameanba}, $\acov{\vy_b}{\data_a} \leftarrow$ \eqref{eq;acovba}\\
% \STATE $\p{\vy_b}{\data_a} \leftarrow$ \eqref{eq:likelihood_b}\\
% \FOR {$j = 0,1$}
...
%  \STATE $\amean{f\st}{\data_{a,b}} \leftarrow$ \eqref{eq:ameanab}, $\acov{f\st}{\data_{a,b}} \leftarrow$ \eqref{eq:acovab}\\
% \STATE {\bfseries return:} $\p{f\st}{\data_{a,b}}\leftarrow$ \eqref{eq:pyab}\\
% \STATE {\bfseries return:} $p(\vy_a) \leftarrow p(\vy_a)\p{\vy_b}{\data_a}$
...
\end{equation}
where $\mean{y}{\data}$ is the method's \emph{a priori} prediction for $y$, and $\sigma_T$ is the noise standard deviation on the faultless training set. Of course, methods using \eqref{eq:thresh} or similar can not provide the posterior probability of a point's faultiness, as our algorithm can. Methods tested include:
\begin{description}
\item[ XGP] A Gaussian process in which we exhaustively search over the faultiness of the last 5 points, and approximate the noise variance of all previous points in the window as having the value $\sigma_f \p{\fault}{\data} + \sigma_n \p{\neg\fault}{\data}$, fixed at the time the point was observed (when data $\data$ was available). Clearly, this method is very much more computationally expensive than the fault bucket algorithm (roughly $2^4$ times more), but offers a useful way to quantify the influence of our approximations.
 \item[ TGP] A Gaussian process in which a point was flagged as a fault using \eqref{eq:thresh}; if faulty, a point was treated as having noise variance $\sigma_f$.
 \item[ ARGP] An auto-regressive Gaussian process (over the last 10 points), with a point identified as faulty using \eqref{eq:thresh}. If faulty, the value for the observation $y$ is taken as $\mean{y}{\data}$ for future predictions.
