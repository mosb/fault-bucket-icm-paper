...
\begin{align*}
\p{y\st}{\data_{a,b}} & = \sum_{i} \sum_{j} \p{y\st}{\data_{a,b}, \vs^{i,j}_{a,b}} \Prob{\vs^{i,j}_{a,b}}{\data_{a}}\,.
\end{align*}
...
& \hspace{3.5cm} - \amean{\vz_b}{\data_{a,b}}^2 \,.
\end{align*}
...
\\
\STATE $p(\vz_a) \leftarrow$\eqref{eq:likelihood_a}
\FOR {$i = 0,1$}
\STATE {\bfseries return:} $\Prob{\vs^i_a}{\data_{a}} \leftarrow$ \eqref{eq:psa}\\
\ENDFOR
 \STATE $\tilde{R}_a \leftarrow$ \eqref{eq:Ra}, $\tilde{T}_a\leftarrow$ \eqref{eq:Ta}, \, $W^{-1}_a \leftarrow$ \eqref{eq:Wa}\\
...
\ENDFOR
\STATE $\tilde{R}_{a,b} \leftarrow$ \eqref{eq:Rab}, $\tilde{T}_{a,b}\leftarrow$ \eqref{eq:Tab}, \, $W^{-1}_{a,b} \leftarrow$ \eqref{eq:Wab}\\
