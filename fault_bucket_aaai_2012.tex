...
\author{ Anonymous Author 1 \And Anonymous Author 2 \And Anonymous Author 3}
...
Water sustainability is one of the most significant issues that humanity faces. Water is essential for life,
but it also has a massive impact on the economy, affecting industries
such as agriculture, mining, power, and more. Population and industrial growth, along with climate change, are beginning to stress
water supplies. Between 1994 and 1999, 26\% of Canadian municipalities reported water shortages despite the fact that Canada
contains 9\% of the world's renewable water supply\footnote{ http://atlas.nrcan.gc.ca/site/english/maps/freshwater/1}.

One of the keys to effective water sustainability involves proper monitoring and analysis. Water is not distributed evenly in space
and time, and being able to measure, predict, and respond to changes in the water supply will allow organizations to allocate supplies
to where they are needed most. To this end, organizations such as Water Survey Canada (\acro{WSC}) and the United States Geological Survey (\acro{USGS})
have set up around 11,000 monitoring stations across North America, many of them reporting telemetry data in real-time. Analyzing this
ever-increasing amount of data by hand is difficult, costly, and time-consuming; there are many opportunities for Machine Learning
to automate this process in order to significantly improve the efficiency and effectiveness of current water-management systems.
...
Our solution is founded upon approximation \ref{app:single_gaussian}. We approximately marginalise the faultiness of old observations, representing the mixture of different Gaussian predictions (each given by a different combination of faultiness) as a single Gaussian. This natural approach is similar to that taken in the related field of switching Kalman filters \citep{murphy1998switching}. We prefer this approximate marginalisation over
...
%\end{align*}
where $\diag \vs$ is the diagonal matrix with diagonal $\vs$. Our approach relies upon storing quantities derived from sums over $i$, and then effecting simple updates in order to arrive at sums over both $i$ and $j$. Hence our approximate marginalisation from previous time steps can be efficiently used to determine an approximate marginalisation for the current time step. 
...
From \eqref{eq:GP_posterior}, we know each of the Gaussians in the sum \eqref{eq:Gaussian_marginal} has relatively simple dependence upon the inverse of $V_a^i$, the precision. We can hence render our analysis anlytic if we make approximation \ref{app:affine}, assuming that
...
The laborious mathematical details of the derivation of the fast, online algorithm built upon these assumptions is omitted for clarity. An outline of our approach is depicted in Algorithm \ref{alg:fault_bucket}.
...
\citep{OsborneAnon}
%\citep[Appendix B]{osbornebayesian} 
...
\,x\st \leftarrow x_{1+\delta}$, fault prior $\pi$.
\\
\STATE {\bfseries return:} $p(\vy_a) \leftarrow$
\acro{data-likelihood-gp$(z_a, \pi)$}\\
\FOR {$i = 0,1$}
\STATE {\bfseries return:} $\Prob{\vs^i_a}{\vy_{a}} \leftarrow$ 
\acro{fault-posterior$\bigl(p(\vy_a), \pi\bigr)$}\\
\ENDFOR
 \STATE $M_a \leftarrow$ 
\acro{approx-marginals}$\bigl(z_a, \{\Prob{\vs^i_a}{\vy_{a}}\}\bigr)$\\
...
\STATE $\p{\vy_b}{\vy_a} \leftarrow$ 
\acro{predictions-gp}$(M_a,\vy_b, \pi)$\\
\STATE $p(\vy_{a,b}) \leftarrow p(\vy_a)\p{\vy_b}{\vy_a}$
\FOR {$j = 0,1$}
...
\acro{update-approx-marginals}\\
\hfill$\bigl(M_a,z_b,\{\Prob{\vs^j_b}{\vy_{a,b}}\}\bigr)$\\
\STATE {\bfseries return:} $\p{f\st}{\vy_{a,b}}\leftarrow$ \eqref{eq:pyab}\\
\STATE {\bfseries return:} $p(\vy_{a} \leftarrow p(\vy_{a})$
...
hyperparameters online using Bayesian Monte Carlo \citep{BZMonteCarlo, OsborneIPSN}, %\citep[Chapter 7]{osbornebayesian},
