...
contains 9\% of the world's renewable water supply (http://atlas.nrcan.gc.ca/site/english/maps/freshwater/1).
...
to where they are needed most. To this end, organizations such as Water Survey Canada (WSC) and the United States Geological Survey (USGS)
...
where $\diag \vs$ is the diagonal matrix with diagonal $\vs$. 
 ...
 We now use approximation \ref{app:single_gaussian}. It is our hope that our predictions
...
To progress, we make approximation \ref{app:independence}; we assume that faults will not persist longer than $|\data_b|$. To be precise, we assume
...
particularly well correlated. On this basis, we make approximation \ref{app:affine}.
